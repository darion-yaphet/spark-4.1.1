...
/**
 * SqlBaseParser 核心流程说明：
 * 1. 语法树构建：将 SqlBaseLexer 生成的标记流解析为强类型的抽象语法树（AST）。
 * 2. 多模式支持：支持标准 SQL 查询（query）、数据定义（DDL）、数据操纵（DML）以及脚本化的复合语句（compoundStatement）。
 * 3. 兼容性控制：通过成员变量（如 legacy_setops_precedence_enabled）动态调整解析优先级，以兼容旧版 Spark SQL 或遵循 ANSI 标准。
 * 4. 复杂表达式解析：处理嵌套查询、Lambda 表达式、窗口函数（Window Functions）以及特定的管道式操作符（Operator Pipe）。
 * 5. 错误捕获：集成特殊的错误捕获规则（如 errorCapturingIdentifier），以产生更友好的语法错误提示。
 */
...
@members {
   ...
   * 为 false 时，根据 SQL 标准，INTERSECT 的优先级高于其他集合操作（UNION、EXCEPT 和 MINUS）。
   */
   ...
   * 为 false 时，带有指数的字面量将被转换为 double 类型而非 decimal 类型。
   */
  public boolean legacy_exponent_literal_as_decimal_enabled = false;

  /**
   * When true, the behavior of keywords follows ANSI SQL standard.
   * 为 true 时，关键字的行为遵循 ANSI SQL 标准。
   */
  public boolean SQL_standard_keyword_behavior = false;

  /**
   * When true, double quoted literals are identifiers rather than STRINGs.
   * 为 true 时，双引号包围的字面量被视为标识符而非字符串。
   */
   ...
   * 为 false 时，参数占位符（? 和 :param）仅允许在常量上下文中使用。
   * 为 true 时，在任何支持字面量的地方都允许使用参数占位符。
   */
   ...
   * 为 false（默认值）时，IDENTIFIER('literal') 在解析时被解析为标识符。
   * 为 true 时，仅允许旧版的 IDENTIFIER(expression) 函数语法。
   * 由 spark.sql.legacy.identifierClause 配置项控制。
   */
...
// inlineTable.
// 与所有其他类型的关系表达式不同，我们不支持针对 inlineTable 的 watermarkClause。
inlineTable
...
// 这仅用于 watermarkClause 有意义的 relationPrimary 中。如果此规则被其他子句引用，
// 请检查 watermarkClause 对该子句是否也有意义。如果无意义，请考虑拆分此规则。
functionTable
...
// 此规则用于明确捕获错误的标识符，例如 test-table，实际上它应该是 `test-table`。
// 在紧随其后的符号不是表达式的情况下，将标识符替换为 errorCapturingIdentifier，
// 否则像 "a-b" 这样有效的表达式可能会被识别为标识符。
errorCapturingIdentifier
    : identifier errorCapturingIdentifierExtra
    ;

// extra left-factoring grammar
// 额外的左提取（left-factoring）语法
...
// simpleIdentifier：类似于 identifier，但不支持 IDENTIFIER('literal') 语法。
// 在 IDENTIFIER() 语法不合适的上下文中使用此标识符：
//   - 命名参数 (:param_name)
//   - 提取字段名 (EXTRACT(field FROM ...))
//   - 其他类似关键字或类似字符串的用途
simpleIdentifier
...
// simpleStrictIdentifier：类似于 strictIdentifier，但不支持 IDENTIFIER('literal') 语法。
simpleStrictIdentifier
...
operatorPipeRightSide
    ...
    // 请注意，WINDOW 子句在 WHERE 管道操作符中是不允许的，但我们在此语法中添加它，
    // 仅仅是为了捕获这种无效语法并抛出特定的专用错误消息。
    ...
    // 以下两种情况分别匹配 PIVOT 或 UNPIVOT 子句。
    // 对于每种情况，我们将另一个子句作为一个选项添加进来，以便在两者同时存在的情况下（这是不允许的）返回高质量的错误消息。
...
// 当 `SQL_standard_keyword_behavior=true` 时，Spark SQL 中有 2 种关键字。
// - 保留关键字：
//     被保留且不能用作表、视图、列、函数、别名等标识符的关键字。
// - 非保留关键字：
//     仅在特定上下文中具有特殊含义，并可在其他上下文中用作标识符的关键字。
//     例如，`EXPLAIN SELECT ...` 是一个命令，但 EXPLAIN 可以在其他地方用作标识符。
// 您可以通过在此文件中搜索 "Start of the keywords list" 来找到完整的关键字列表。
// 下方列出了非保留关键字。不在该列表中的关键字即为保留关键字。
ansiNonReserved
...
// 当 `SQL_standard_keyword_behavior=false` 时，Spark SQL 中有 2 种关键字。
// - 非保留关键字：
//     定义与 `SQL_standard_keyword_behavior=true` 时相同。
// - 严格非保留关键字：
//     非保留关键字的严格版本，不能用作表别名。
// 您可以通过在此文件中搜索 "Start of the keywords list" 来找到完整的关键字列表。
// 严格非保留关键字列在 `strictNonReserved` 中。
// 非保留关键字列在 `nonReserved` 中。
// 这两者共同包含了所有的关键字。
strictNonReserved
    : ANTI
    | CROSS
